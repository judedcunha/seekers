...
import nets.TinyFaceNet as TinyFaceNet
import nets.FaceNet as FaceNet
...
def get_parser():
    ...
    parser.add_argument('--model_type', default=0, help='FaceNet or TinyFaceNet')
...
if __name__ == '__main__':
    with tf.Graph().as_default():
        ...
        if args.model_type == 0:
            prelogits, net_points = FaceNet.inference(images=inputs,
                                                            phase_train=phase_train_placeholder,
                                                            weight_decay=args.weight_decay)
        else:
            prelogits, net_points = TinyFaceNet.inference(images=inputs,
                                                                phase_train=phase_train_placeholder,
                                                                weight_decay=args.weight_decay)
        # record the network architecture
        hd = open("./arch/txt/FaceNet_Arch.txt", 'w')
        ...
        # FaceNet_vars = [v for v in tf.trainable_variables() if v.name.startswith('FaceNet')]
            ...
            while True:
                try:
                    ...
                    if count > 0 and count % args.ckpt_interval == 0:
                        filename = 'FaceNet_epoch_{:d}_iter_{:d}'.format(i, count) + '.ckpt'
